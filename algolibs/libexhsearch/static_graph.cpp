...
std::vector<Location> StaticGraph::neighbors(const Location & location) const
{
	...
	for (auto out_path_iterator = node.out_paths.cbegin(); out_path_iterator != node.out_paths.cend(); out_path_iterator++) {
		...
		auto potential_location = location + offsetFromOutPath(out_path);
...
size_t StaticGraph::getNumberOfNodes() const
{
...
bool StaticGraph::hasOutPath(const Node & node, const OutPathType & out_path) const
{
	return node.out_paths.find(out_path) != std::string::npos;
}

bool StaticGraph::isInRange(const Location & location) const
{
...
StaticGraph::OutPathType StaticGraph::mirrorOutPath(OutPathType out_path)
{
	switch (out_path)
	{
	case 'N':
...
Location::OffsetType StaticGraph::offsetFromOutPath(OutPathType out_path)
{
	switch (out_path)
	{
	case 'N':
